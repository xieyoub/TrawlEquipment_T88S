...
u16 NixieTubValue[3] = {0};   //三个数码管的值

u16 SilentTime=0;             //  静默时间
u8 State=0;                   //当前状态，0:无写码状态，1：正在写码       
u8 UsartUsing = 1;            //0串口正在等待  ，1串口空闲
...
void * MsgGrp[20];			//消息队列存储地址,最大支持256个消息
void * MsgWrite[10];  
OS_EVENT * q_msg;			//消息队列
OS_EVENT * msg_hand;	
OS_EVENT * uart;
...
void start_task(void *pdata)
{
	...
	uart = OSSemCreate(1); //串口使用信号量
	msg_hand = OSMboxCreate((void*) 0); //握手串口接收邮箱
	q_msg=OSQCreate(&MsgGrp[0],20);	//创建消息队列
	msg_receive = OSMboxCreate((void*) 0); //第一次注入邮箱
	msg_write=OSQCreate(&MsgWrite[0],10);	//第一次注入队列
